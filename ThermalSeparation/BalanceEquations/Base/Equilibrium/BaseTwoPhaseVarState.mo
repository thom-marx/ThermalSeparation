...
/*** ENERGY BALANCE ***/
  if n==1 then
    der(n_mol_L[1]*u_l[1]+n_mol_V[1]*u_v[1]) = Ndot_l_in*h_downStreamIn_act+Ndot_v_in*h_upStreamIn_act   -Ndot_l[1]*h_downStreamOut_act-Ndot_v[n]*h_upStreamOut_act  - Qdot_wall[1] + Vdot_l_feed[1]*sum(c_l_feed[1,:])*h_l_feed[1] - Vdot_le[1]*sum(c_l[1,:])*h_l[1] +Qdot_reac[1]+ Vdot_v_feed[1]*sum(c_v_feed[1,:])*h_v_feed[1] + Ndot_source_startUp[1]*h_v[1];
    Edot_l_transfer[1]=0;
  else
   der(n_mol_L[1]*u_l[1]+n_mol_V[1]*u_v[1]) = Vdot_l[2]*sum(c_l[2,:])*h_l[2] +Ndot_v_in*h_upStreamIn_act -Ndot_l[1]*h_downStreamOut_act  - Vdot_v[1]*sum(c_v[1,:])*h_v[1] - Qdot_wall[1] + Vdot_l_feed[1]*sum(c_l_feed[1,:])*h_l_feed[1] - Vdot_le[1]*sum(c_l[1,:])*h_l[1] + Qdot_reac[1]+ Vdot_v_feed[1]*sum(c_v_feed[1,:])*h_v_feed[1] + Ndot_source_startUp[1]*h_v[1];
   for j in 2:n-1 loop
     der(n_mol_L[j]*u_l[j]+n_mol_V[j]*u_v[j]) = Vdot_l[j+1]*sum(c_l[j+1,:])*h_l[j+1] +Vdot_v[j-1] * sum(c_v[j-1,:])*propsVap[j-1].h - Vdot_l[j]*sum(c_l[j,:])*h_l[j] - Vdot_v[j]*sum(c_v[j,:])*h_v[j] -Qdot_wall[j]+ Vdot_l_feed[j]*sum(c_l_feed[j,:])*h_l_feed[j] +Vdot_le[j-1]*sum(c_l[j-1,:])*h_l[j-1] - Vdot_le[j]*sum(c_l[j,:])*h_l[j] + Qdot_reac[j]+ Vdot_v_feed[j]*sum(c_v_feed[j,:])*h_v_feed[j]  + Ndot_source_startUp[j]*h_v[j];
   end for;
   der(n_mol_L[n]*u_l[n]+n_mol_V[n]*u_v[n])= Ndot_l_in*h_downStreamIn_act +Vdot_v[n-1] * sum(c_v[n-1,:])*propsVap[n-1].h - Vdot_l[n]*sum(c_l[n,:])*h_l[n] -Ndot_v[n]*h_upStreamOut_act - Qdot_wall[n] + Vdot_l_feed[n]*sum(c_l_feed[n,:])*h_l_feed[n] + Vdot_le[n-1]*sum(c_l[n-1,:])*h_l[n-1]+ Qdot_reac[n] + Vdot_v_feed[n]*sum(c_v_feed[n,:])*h_v_feed[n] + Ndot_source_startUp[n]*h_v[n];
   for i in 1:n loop
     Edot_l_transfer[i]=0;
   end for;
