...
model counterFlowHeatExchangerH2O
...
// components
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_b(redeclare package Medium = 
        MediumLiquidCold) 
    annotation (Placement(transformation(extent={{92,56},{112,76}})));
...
//der(TwallCold) = heatFlowHotLiquidTube.Q_flow+heatFlowColdLiquidTube.Q_flow;
 ...
 -alphaHot*(hotLiquidIn.T-TwallHot)+der(TwallHot)*plateMetalMass*cpMetal/surfaceArea  = -alphaCold*(TwallHot - coldLiquidOut.T);
  ...
  connect(hotLiquidIn, hotLiquidTube.liquidIn) 
                                             annotation (Line(
      points={{100,-70},{58,-70},{58,30},{-54,30},{-54,19.6}},
      color={255,0,0},
      smooth=Smooth.None));
  connect(hotLiquidTube.liquidOut, hotLiquidOut) 
                                               annotation (Line(
      points={{-54,-19.6},{-54,-70},{-100,-70}},
      color={255,0,0},
      smooth=Smooth.None));
  ...
  connect(valveLinear.port_b, port_b) annotation (Line(
      points={{82,-10},{84,-10},{84,66},{102,66}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(coldLiquidIn.port, port_a) 
                                    annotation (Line(
      points={{-54,66},{-78,66},{-78,62},{-100,62}},
      color={0,127,255},
      smooth=Smooth.None));
  ...
  connect(port_a, volume.ports[1]) annotation (Line(
      points={{-100,62},{-30,62},{-30,-10},{38,-10}},
      color={0,127,255},
      smooth=Smooth.None));
...
//           color={255,0,0},
//           thickness=0.5)}));
end counterFlowHeatExchangerH2O;
