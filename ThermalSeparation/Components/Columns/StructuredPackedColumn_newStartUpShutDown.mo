...
// eps in m3 Solid / m3 gesamt
  ...
  replaceable model Reaction = ThermalSeparation.Reaction.NoReaction
    constrainedby ThermalSeparation.Reaction.BaseReaction "model for chemical reaction"                 annotation(choicesAllMatching=true);
...
propsLiq = mediumLiquid.properties, propsVap = mediumVapour.properties,
 ...
 stateLiq=mediumLiquid.state, k=k,delay_startUp=delay_startUp,StartUp_CCS=StartUp_CCS,smooth_startUp=smooth_startUp,lowBoilingPoint=lowBoilingPoint, stateVap=mediumVapour.state, Ndot_l_transfer=Ndot_l_transfer,
 eps_liq=eps_liq, eta_comp=mediumLiquid.eta_comp, x_l=x_l, p_sat=p_sat,x_v_in=x_v_in,before_transition=before_transition, gamma=activityCoeff[:].gamma,
 redeclare final model ThermoEquilibrium =     ThermoEquilibrium,
 c_v_star=c_v_star,redeclare replaceable model HomotopyMethod = HomotopyMethod,x_vap_liq=fill(1/nS,n,nS));

  final parameter Integer aux[  :] = {1,3,6,10,15, 21, 28, 36, 45};

  parameter Integer n_elements(min=1)=2 "number of discrete elements or number of equilibrium stages (for equilibrium film model)";

//Initialization
  parameter Boolean wettedInitial = true "true if package material is already wetted (i.e. if liquid enters the column there is immediately a liquid stream leaving the column)"
                                                                                            annotation(Dialog(tab="Initialization", group="Initial liquid content"));
   parameter Real eps_liq_start = 0.06 "start value for liquid content if it is not exactly wetted but with more or less liquid"
  ...
  replaceable model Holdup =
    ...
    constrainedby ThermalSeparation.Holdup.StructuredPackedColumn.BaseHoldup "calculation of holdup inside the column"                          annotation(choicesAllMatching);
  Holdup holdup(
      redeclare package MediumLiquid = MediumLiquid,
  redeclare package MediumVapour=MediumVapour,
  ...
  Real F[n]= Vdot_v/geometry.A .* rho_v.^0.5 "F-Factor to check operating range, unit: Pa^0.5";
  Real liquidLoad[n] = Vdot_l/geometry.A*3600 "liquid load to check operating range, unit: m3/m2/h";
...
T_star = filmModel.T_star;
      ...
      annotation (extent=[-70,-100; -50,-80], Diagram(coordinateSystem(
...
Model eines Packungskolonnenabschnitts mit einem Dampf- und einem Fluessigstrom.
    ...
    Icon(coordinateSystem(preserveAspectRatio=false, extent={{-100,-100},{100,
            100}}),
         graphics));
end StructuredPackedColumn_newStartUpShutDown;
