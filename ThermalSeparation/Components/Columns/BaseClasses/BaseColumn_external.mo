...
replaceable package MediumLiquid =
    ...
    "medium to be used in liquid phase"                                                                                                  annotation(choicesAllMatching);
       MediumLiquid.BaseProperties mediumLiquid[n](each T0=T_ref,  p=p_hyd[1:n], T=T_l, x= x_l);
 MediumLiquid.BaseProperties mediumLiquidIn(T0=T_ref,  p=p_hyd[n+1], T=T_l_in, x=x_l_in);
 MediumLiquid.BaseProperties mediumLiquidStar[n](each T0=T_ref, p=p_hyd[1:n], T=T_star, x=x_l_star);
...
//Variables upStream
  ...
  SI.Temperature T_star[n](start=T_l);
...
Real n_tot[n,nS];
SI.MoleFraction x_vap_liq[n,nS];
...
equation
  ...
  //Verknüpfung der Konnektorgrößen mit Variablen
  //upstream
...
if considerStartUp then
  for j in 1:n loop
    when p_bub[j] + friggelfaktor >= p_initial then // der Druck, den auch das Medienmodell sieht, friggelfaktor, weil sonst Umschalten zu spät
      startUp[j] = false;
    end when;
