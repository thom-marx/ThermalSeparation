...
package BaseClasses "base classes for columns"
  partial model BaseColumn_newStartUpShutDown "Gesamtmolbilanz, x, h, Ndot im Konnektor"
    ...
    parameter Boolean x_l_profile = false annotation (
      Dialog(tab = "Initialization"));
    parameter Boolean x_v_profile = false annotation (
      Dialog(tab = "Initialization"));
    parameter SI.MoleFraction x_l_start_const[nSL] = fill(1 / nSL, nSL) annotation (
      Dialog(enable = not x_l_profile, tab = "Initialization"));
    parameter SI.MoleFraction x_v_start_const[nSV] = fill(1 / nSV, nSV) annotation (
      Dialog(enable = not x_v_profile, tab = "Initialization"));
    parameter SI.MoleFraction x_l_start_in[nSL] = fill(1 / nSL, nSL) annotation (
      Dialog(enable = x_l_profile, tab = "Initialization"));
    parameter SI.MoleFraction x_l_start_out[nSL] = fill(1 / nSL, nSL) annotation (
      Dialog(enable = x_l_profile, tab = "Initialization"));
    parameter SI.MoleFraction x_v_start_in[nSV] = fill(1 / nSV, nSV) annotation (
      Dialog(enable = x_v_profile, tab = "Initialization"));
    parameter SI.MoleFraction x_v_start_out[nSV] = fill(1 / nSV, nSV) annotation (
      Dialog(enable = x_v_profile, tab = "Initialization"));
    ...
    parameter Real x_total_start[nSV] = fill(1 / nSV, nSV) "total mole fraction in system (vapour and liquid), component ordering as in vapour medium" annotation (
      Dialog(tab = "Initialization"));
    parameter Boolean T_l_profile = false annotation (
      Dialog(tab = "Initialization"));
    parameter Boolean T_v_profile = false annotation (
      Dialog(tab = "Initialization"));
    parameter SI.Temperature T_vap_start_bottom = 300 annotation (
      Dialog(tab = "Initialization"));
    parameter SI.Temperature T_vap_start_top = 300 annotation (
      Dialog(tab = "Initialization"));
    parameter SI.Temperature T_liq_start_bottom = 300 annotation (
      Dialog(tab = "Initialization"));
    parameter SI.Temperature T_liq_start_top = 300 annotation (
      Dialog(tab = "Initialization"));
    parameter SI.Temperature T_vapour_start = 300 annotation (
      Dialog(tab = "Initialization"));
    ...
    replaceable package MediumVapour = ThermalSeparation.Media.H2O_CO2_Vap constrainedby ThermalSeparation.Media.BaseMediumVapour "medium to be used in vapour phase" annotation (
       choicesAllMatching);
    ...
    replaceable package MediumLiquid = ThermalSeparation.Media.H2O_CO2_MEA_Liq constrainedby ThermalSeparation.Media.BaseMediumLiquid "medium to be used in liquid phase" annotation (
       choicesAllMatching);
    ...
    ThermalSeparation.Interfaces.GasPortOut upStreamOut(redeclare package Medium = MediumVapour) annotation (
      Placement(transformation(extent = {{-80, 80}, {-60, 100}}, rotation = 0), iconTransformation(extent = {{-80, 80}, {-60, 100}})));
    ThermalSeparation.Interfaces.LiquidPortIn downStreamIn(redeclare package Medium = MediumLiquid) annotation (
    ...
    replaceable model ThermoEquilibrium = ThermalSeparation.PhaseEquilibrium.RealGasActivityCoeffLiquid constrainedby ThermalSeparation.PhaseEquilibrium.BasePhaseEquilibrium "model for phase equilibrium" annotation (
       choicesAllMatching = true);
    ThermoEquilibrium bubblePressure[n](each nS = nS, each mapping = mapping, redeclare replaceable package MediumVapour = MediumVapour, redeclare replaceable package MediumLiquid = MediumLiquid, p = ones(n) * p_initial, T = T_l, x_v = x_v, x_l = x_l, p_sat = p_sat_bulk, v_v = MM_v ./ rho_v, each x_vap_liq = fill(1, nS));
protected
    ...
    final parameter SI.Mass mass_solid[n] = A * H / n * (1 - eps) * rho_solid;
...
public
    ...
    parameter Boolean considerStartUp = false "true if StartUp is to be considered" annotation (
      Dialog(tab = "StartUp"));
    parameter Boolean considerShutDown = false "true if ShutDown is to be considered" annotation (
      Dialog(tab = "ShutDown"));
    parameter Boolean StartUp_CCS = false "true if StartUp of carbon capture plant is to be considered" annotation (
      Dialog(tab = "StartUp", enable = considerStartUp));
    parameter Boolean switchingCondition_Boiling = true "true if boiling state is switching condition" annotation (
      Dialog(tab = "StartUp", enable = StartUp_CCS));
    parameter Boolean switchingCondition_Absorber_x_v = false "true if vapour composition is switching condition" annotation (
      Dialog(tab = "StartUp"),
      enable = StartUp_CCS);
    parameter Real x_v_switch = 0.05 "vapour mole fraction value which is to be achieved" annotation (
      Dialog(tab = "StartUp", enable = switchingCondition_Absorber_x_v));
    parameter Integer componentNumber = 3 "number of vapour component number in model" annotation (
      Dialog(tab = "StartUp", enable = switchingCondition_Absorber_x_v));
    ...
    parameter Real k = 0.2e-3 "large value for steep omega" annotation (
      Dialog(tab = "StartUp"));
    parameter Real friggelfaktor = 0.0002e5 "value added to p_bub to shift switching time" annotation (
      Dialog(tab = "StartUp"));
    parameter Boolean smooth_startUp = false "true if smooth switching is to be considered" annotation (
      Dialog(tab = "StartUp", group = "Smooth Start-Up"));
    parameter Real delay_startUp = 200 "time delay for smooth startUp" annotation (
      Dialog(tab = "StartUp", group = "Smooth Start-Up", enable = smooth_startUp));
    parameter Boolean lowBoilingPoint[nSV] = fill(false, nSV) "true if substance has low boiling point" annotation (
      Dialog(tab = "StartUp"));
    parameter Real y_PID = 10 "maximal value for supply startUp PID controller" annotation (
      Dialog(tab = "StartUp"));
    parameter Real Vdot_startUp_pressure = 0.005 "value when supply PID controller is switched off" annotation (
      Dialog(tab = "StartUp"));
    ...
    Modelica.Blocks.Interfaces.BooleanInput ShutDown_signal annotation (
      Placement(transformation(extent = {{-102, -50}, {-92, -40}})));
    ThermalSeparation.Components.Columns.BaseClasses.InternalStepSequence_StateGraph1 internalStepSequence[n] annotation (
      Placement(transformation(extent = {{-20, 0}, {0, 20}})));
    Modelica.Blocks.Routing.BooleanReplicator shutDownReplicator(nout = n) annotation (
      Placement(transformation(extent = {{-24, -48}, {-16, -42}})));
    Modelica.Blocks.Routing.BooleanReplicator startUpReplicator(nout = n) annotation (
      Placement(transformation(extent = {{-72, -64}, {-62, -58}})));
    Modelica.Blocks.Sources.BooleanExpression booleanExpression(y = Vdot_v_in > Vdot_startUp_pressure) annotation (
      Placement(transformation(extent = {{-64, 6}, {-50, 18}})));
    Modelica.Blocks.Routing.BooleanReplicator endPhase1(nout = n) annotation (
      Placement(transformation(extent = {{-38, 8}, {-30, 14}})));
  initial algorithm
  /*** linear concentration profile at t=0 ***/
    if n == 1 then
      ...
      for j in 1:n loop
        for i in 1:nSV loop
          if x_v_profile then
            x_v_start[j, i] := (x_v_start_out[i] - x_v_start_in[i]) / (n - 1) * j + x_v_start_in[i] - (x_v_start_out[i] - x_v_start_in[i]) / (n - 1);
          else
        ...
        for i in 1:nSL loop
          if x_l_profile then
            x_l_start[j, i] := (x_l_start_out[i] - x_l_start_in[i]) / (n - 1) * j + x_l_start_in[i] - (x_l_start_out[i] - x_l_start_in[i]) / (n - 1);
          else
  ...
  equation
    connect(StartUp_signal, startUpReplicator.u) annotation (
      Line(points = {{-97, -61}, {-87.5, -61}, {-73, -61}}, color = {255, 0, 255}));
    connect(endPhase1.u, booleanExpression.y) annotation (
      Line(points = {{-38.8, 11}, {-43.4, 11}, {-43.4, 12}, {-49.3, 12}}, color = {255, 0, 255}));
    connect(ShutDown_signal, shutDownReplicator.u) annotation (
      Line(points = {{-97, -45}, {-61.5, -45}, {-24.8, -45}}, color = {255, 0, 255}));
    if switchingCondition_Boiling then
      for i in 1:n loop
    ...
    connect(shutDownReplicator.y, internalStepSequence.shutDown_signal) annotation (
      Line(points = {{-15.6, -45}, {-6.2, -45}, {-6.2, 0}}, color = {255, 0, 255}));
    ...
    connect(endPhase1.y, internalStepSequence.endPhase1) annotation (
      Line(points = {{-29.6, 11}, {-24.8, 11}, {-24.8, 11.2}, {-19.8, 11.2}}, color = {255, 0, 255}));
    annotation (
      Icon(graphics),
...
<p>The mass transfer equations and the equations for the thermodynamic equilibrium are provided in the film model, which is instantiated in the column specific classes <a href=\"Modelica://ThermalSeparation.Components.Columns.StructuredPackedColumn\">StructuredPackedColumn</a>, <a href=\"Modelica://ThermalSeparation.Components.Columns.RandomPackedColumn\">RandomPackedColumn</a>, <a href=\"Modelica://ThermalSeparation.Components.Columns.TrayColumn\">TrayColumn</a> and <a href=\"Modelica://ThermalSeparation.Components.Columns.SprayColumn\">SprayColumn</a>.</p>
  ...
  model InternalStepSequence
    Modelica_StateGraph2.Step ColdState(initialStep = true, nOut = 1, nIn = 1, use_activePort = false) annotation (
      Placement(transformation(extent = {{-36, 32}, {-28, 40}})));
    Modelica_StateGraph2.Transition T1(use_conditionPort = true) annotation (
      Placement(transformation(extent = {{-36, 8}, {-28, 16}})));
    Modelica_StateGraph2.Step HeatUpState(nOut = 1, initialStep = false, nIn = 1) annotation (
      Placement(transformation(extent = {{-4, -4}, {4, 4}}, rotation = 0, origin = {-32, -14})));
    Modelica_StateGraph2.Transition T2(use_conditionPort = true) annotation (
      Placement(transformation(extent = {{-4, -4}, {4, 4}}, rotation = 0, origin = {-32, -38})));
    Modelica_StateGraph2.Step NormalOperation(nOut = 1, initialStep = false, nIn = 1) annotation (
      Placement(transformation(extent = {{-4, -4}, {4, 4}}, rotation = 90, origin = {-4, -54})));
    Modelica_StateGraph2.Transition T3(use_conditionPort = true, delayedTransition = true, waitTime = 1) annotation (
      Placement(transformation(extent = {{-4, -4}, {4, 4}}, rotation = 180, origin = {20, -40})));
    Modelica_StateGraph2.Step ShutDown(initialStep = false, nIn = 1, nOut = 1) annotation (
      Placement(transformation(extent = {{-4, -4}, {4, 4}}, rotation = 180, origin = {20, 12})));
    Modelica_StateGraph2.Transition T4(use_conditionPort = true) annotation (
      Placement(transformation(extent = {{-4, -4}, {4, 4}}, rotation = 270, origin = {-2, 60})));
    Modelica.Blocks.Interfaces.BooleanInput shutDown_signal annotation (
      Placement(transformation(extent = {{-16, -16}, {16, 16}}, rotation = 90, origin = {38, -100})));
    Modelica.Blocks.Interfaces.BooleanInput StartUp_signal1 annotation (
      Placement(transformation(extent = {{-17, -17}, {17, 17}}, rotation = 90, origin = {81, -99})));
    Modelica.Blocks.Interfaces.BooleanInput endPhase1 annotation (
      Placement(transformation(extent = {{-16, -16}, {16, 16}}, rotation = 0, origin = {-98, 12})));
    Modelica.Blocks.Interfaces.BooleanInput endPhase2 annotation (
      Placement(transformation(extent = {{-2, -2}, {2, 2}}, rotation = 0, origin = {-44, -38})));
  equation
    connect(ColdState.outPort[1], T1.inPort) annotation (
      Line(points = {{-32, 31.4}, {-32, 26}, {-32, 20}, {-32, 16}}, color = {0, 0, 0}));
    connect(HeatUpState.inPort[1], T1.outPort) annotation (
      Line(points = {{-32, -10}, {-32, -10}, {-32, 7}}, color = {0, 0, 0}));
    connect(T2.inPort, HeatUpState.outPort[1]) annotation (
      Line(points = {{-32, -34}, {-32, -32}, {-32, -25.4}, {-32, -18.6}}, color = {0, 0, 0}));
    connect(NormalOperation.inPort[1], T2.outPort) annotation (
      Line(points = {{-8, -54}, {-32, -54}, {-32, -43}}, color = {0, 0, 0}));
    connect(NormalOperation.outPort[1], T3.inPort) annotation (
      Line(points = {{0.6, -54}, {0.6, -54}, {20, -54}, {20, -44}}, color = {0, 0, 0}));
    connect(ShutDown.inPort[1], T3.outPort) annotation (
      Line(points = {{20, 8}, {20, -22}, {20, -35}}, color = {0, 0, 0}));
    connect(T4.outPort, ColdState.inPort[1]) annotation (
      Line(points = {{-7, 60}, {-32, 60}, {-32, 40}}, color = {0, 0, 0}));
    connect(T4.inPort, ShutDown.outPort[1]) annotation (
      Line(points = {{2, 60}, {20, 60}, {20, 16.6}}, color = {0, 0, 0}));
    connect(shutDown_signal, T3.conditionPort) annotation (
      Line(points = {{38, -100}, {38, -40}, {25, -40}}, color = {255, 0, 255}));
    connect(StartUp_signal1, T4.conditionPort) annotation (
      Line(points = {{81, -99}, {81, 80}, {-2, 80}, {-2, 65}}, color = {255, 0, 255}));
    connect(endPhase1, T1.conditionPort) annotation (
      Line(points = {{-98, 12}, {-37, 12}}, color = {255, 0, 255}));
  ...
  partial model FeedColumn_newStartUpShutDown "column with optional liquid and or vapour feed inlets"
    ...
    InternalFeedPort internalFeedPort[n](feedLiquidInternal(redeclare each package Medium = MediumLiquid), feedVapourInternal(redeclare each package Medium = MediumVapour));
protected
...
public
    parameter Boolean hasLiquidFeed = false "true, if there exist a liquid feed" annotation (
      Dialog(tab = "Feed", group = "Liquid Feed"));
    parameter Integer numberLiquidFeeds(min = 0, max = n) = 1 annotation (
      Dialog(enable = hasLiquidFeed, tab = "Feed", group = "Liquid Feed"));
    parameter Integer[numberLiquidFeeds] stageLiquidFeed = {2} "number of stage where feed enters the column" annotation (
      Dialog(enable = hasLiquidFeed, tab = "Feed", group = "Liquid Feed"));
    final parameter Integer numberLiquidFeedsInternal(min = 0, max = n) = if hasLiquidFeed then numberLiquidFeeds else 0;
    Interfaces.LiquidPortIn[numberLiquidFeeds] feedLiquid(redeclare each package Medium = MediumLiquid) if hasLiquidFeed annotation (
      Placement(transformation(extent = {{-94, -14}, {-74, 6}}, rotation = 0), iconTransformation(extent = {{-94, -14}, {-74, 6}})));
...
public
    parameter Boolean hasVapourFeed = false "true, if there exist a liquid feed" annotation (
      Dialog(tab = "Feed", group = "Vapour Feed"));
    parameter Integer numberVapourFeeds(min = 0, max = n) = 1 annotation (
    ...
    ThermalSeparation.Interfaces.LiquidPortOut[numberLiquidFeeds] feedLiquid_dummy2(redeclare each package Medium = MediumLiquid) if not hasLiquidFeed;
    SourcesSinks.SourceGas sourceGas[numberVapourFeeds](each use_Flow = false, redeclare each package Medium = MediumVapour, each T = 293.15, each x = x_v_start_const, each Flow = 1) if not hasVapourFeed;
    SourcesSinks.SourceLiquid sourceLiquid[numberLiquidFeeds](each use_Flow = false, redeclare each package MediumLiquid = MediumLiquid, each T = 293.15, each x = x_l_start_const, each Flow = 1) if not hasLiquidFeed;
    SourcesSinks.SinkGas sinkGas[numberVapourFeeds](redeclare each package Medium = MediumVapour, each p = 100000) if not hasVapourFeed;
    SourcesSinks.SinkLiquid sinkLiquid[numberLiquidFeeds](redeclare each package Medium = MediumLiquid, each p = 100000) if not hasLiquidFeed;
