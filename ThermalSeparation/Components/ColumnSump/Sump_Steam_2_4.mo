...
Real N_Eth =  c_l[1]*V_l + c_v[1]*V_v;
      ...
      Interfaces.LiquidPortIn liquidIn( nS=nSL,x=x_l_in,c=c_l_in,T=T_in,Vdot=Vdot_in_l,p=p_system) 
    ...
    Interfaces.GasPortIn gasPortIn( nS=1,p=p_steam,Vdot=vdot_steam_in,T=dummy2) annotation (Placement(transformation(extent={{44,6},{64,26}}),
        iconTransformation(extent={{42,26},{52,16}})));
  Interfaces.GasPortOut gasPortOut( nS=1,p=p_steam,Vdot=-vdot_steam_out,T=dummy2,p_medium=p_steam) annotation (Placement(transformation(extent={{-86,6},
            {-66,26}}),            iconTransformation(extent={{-76,16},{-66,
            26}})));
Interfaces.LiquidPortOut liquidPortOut( nS=1,p=p_steam,Vdot=-vdot_water_out,T=dummy2) annotation (Placement(transformation(extent={{-86,-42},
            {-66,-22}}),           iconTransformation(extent={{-76,-32},{-66,
            -22}})));

/* heat transfer */
input SI.Pressure p_steam;
input SI.VolumeFlowRate vdot_steam_in;
SI.VolumeFlowRate vdot_water_out;
SI.VolumeFlowRate vdot_steam_out;
input SI.Temperature dummy2;

replaceable model HeatTransferModel = 
ThermalSeparation.Components.ColumnSump.HeatTransferModel.dT_m constrainedby
...
vdot_water_out=heatTransferModel.vdot_l_out;
vdot_steam_out=heatTransferModel.vdot_v_out;
