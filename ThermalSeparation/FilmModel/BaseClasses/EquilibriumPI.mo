...
model EquilibriumPI "equilibrium model using PI controller"
  ...
  ThermoEquilibrium thermoEquilibrium[n](x_vap_liq=x_vap_liq,
    each nS=nS, each mapping =                                    mapping,
      ...
      redeclare replaceable package MediumLiquid =    MediumLiquid,
       ...
       T=T_vap,
       k=k_vap,
       y_start=K_start_vap, u= max(vector_vap[1,:])+1e-2);
     ThermalSeparation.Utilities.PI_Input PI_liq(initType=Modelica.Blocks.Types.Init.InitialOutput,
       y_start = K_start_liq,
       k=k_liq,
       T=T_liq, u= max(vector_liq[1,:])+1e-2);

  parameter Real k_vap = 1e4 "gain of PI controller" annotation(Dialog(tab="Heat and Mass Transfer", group = "Equilibrium"));
  parameter Real T_vap = 0.01 "time constant of PI controller" annotation(Dialog(tab="Heat and Mass Transfer", group = "Equilibrium"));
  parameter Real K_start_vap=1 "start value for output of PI controller" annotation(Dialog(tab="Heat and Mass Transfer", group = "Equilibrium"));
  parameter Real k_liq = 1e4 "gain of PI controller" annotation(Dialog(tab="Heat and Mass Transfer", group = "Equilibrium"));
  parameter Real T_liq=0.1 "time constant of PI controller" annotation(Dialog(tab="Heat and Mass Transfer", group = "Equilibrium"));
  parameter Real K_start_liq=1e-2 "start value for output of PI controller" annotation(Dialog(tab="Heat and Mass Transfer", group = "Equilibrium"));

    /*** film is calculated steady-state, no source term due to reaction ***/
  parameter Real factorHT = 1e8
 ...
 /*** thermodynamic equilbrium also in the bulk phases ***/
   for j in 1:n loop
    for i in 1:nSV loop
         ...
         Ndot_v_transfer[j,i] =  - A_I[j]*PI_vap.y*(x_v[j,i] - x_v_star[j,i]);

       end if;
     end for;
     //Liquid side
    for i in 1:nSL loop
      Ndot_l_transfer[j,i] =  -A_I[j] * PI_liq.y* (x_l[j,i] - x_l_star[j,i]);
...
for j in 1:n loop
    ...
    Qdot_l_transfer[j] = factorHT* (T_star[j] - T_l[j]);
    Qdot_v_transfer[j] = factorHT*(T_star[j] - T_v[j]);
