...
Vdot[j] =  max(0,sign(p[j]-p[j+1])*sqrt(abs(p[j]-p[j+1]) *4*eps_dash[j]^4.65*dp_dash[j] /(3*f0_dash[j]*(1-eps_dash[j])*rho_v_av[j]*h))*geometry.A);
//Vdot[j] =  sign(p[j]-p[j+1])*sqrt(abs(p[j]-p[j+1]) *4*geometry.eps^4.65*geometry.dp /(3*f0[j]*(1-geometry.eps)*rho_v_av[j]*h))*geometry.A;
end for;

/*** outlet: half of the n-th discreet element ***/

Vdot[n] = max(0,sign(p[n]-p[n+1])*sqrt(max(0,abs(p[n]-p[n+1]))*4*eps_dash_out^4.65*dp_dash_out/(3*f0_dash_out*(1 - eps_dash_out)*rho_v[n]*h/2))*geometry.A);
//Vdot[n] = sign(p[n]-p[n+1])*sqrt(abs(p[n]-p[n+1])*4 *geometry.eps^4.65*geometry.dp /(3*f0_out*(1-geometry.eps)*rho_v_calc[n]*h))*geometry.A;
...
//p_v_in = p[1]+sign(Vdot_in)*Vdot_in*Vdot_in/geometry.A^2*3/4*f0_in*(1-geometry.eps)/geometry.eps^4.65/rho_v_calc[1]/geometry.dp*h/2;
