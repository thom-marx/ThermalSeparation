...
equation
 ...
 //  A[j]*w_sup_v[j]^2 + B[j]*w_sup_v[j] - (p[j] - p[j + 1])/(geometry.H/n)*(1-(0.164 + 71.35*geometry.S)*(4*Ft[j]/geometry.S)^(2/3)  *(3*eta_l_calc[j] * w_sup_l[j]/(rho_l[j]*sin_theta*geometry.eps*9.81*((rho_l[j]-rho_v_calc[j])/rho_l[j]*max(0.1,(1-(p[j] - p[j + 1])/(geometry.H/n)/deltaP_flood)))))^(1/3))^5=0;
...
//   Vdot[n] = max(0,sign(p[n] - p[n + 1])*max(0, (abs(p[n] - p[n + 1])/(K/2))));
for j in n:n loop
  //  A[j]*w_sup_v[j]^2 + B[j]*w_sup_v[j] - (p[j] - p[j + 1])*2/(geometry.H/n)*(1-(0.164 + 71.35*geometry.S)*(4*Ft[j]/geometry.S)^(2/3)  *(3*eta_l_calc[j] * w_sup_l[j]/(rho_l_calc[j]*sin_theta*geometry.eps*9.81*((rho_l_calc[j]-rho_v_calc[j])/rho_l_calc[j]*max(0.1,(1-(p[j] - p[j + 1])*2/(geometry.H/n)/deltaP_flood/2)))))^(1/3))^5=0;
