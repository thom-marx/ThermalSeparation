...
package Functions
  ...
  algorithm
    h_der := dT*
      ThermalSeparation.Media.IdealGasMixtures.BaseClasses.Common.Functions.cp_Tlow(
      data, T);
    annotation(Inline=true,smoothOrder=2);
  end h_Tlow_der;
...
<p>T. Skoglund, Lund, Sweden, 2004-08-31</p>
  ...
  algorithm
    lambda := if method == 1 then eta*(Cp - Modelica.Constants.R/MM + (9/4)*Modelica.Constants.R/MM) else eta*(Cp
       - Modelica.Constants.R/MM)*(1.32 + 1.77/((Cp/Modelica.Constants.R) - 1.0));
    annotation (smoothOrder=2,
                Documentation(info="<html>
<p>
