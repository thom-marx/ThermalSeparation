...
model Concentration "using concentrations for equilibrium constants"
...
extends BaseEquilibriumConstant;
 ...
 parameter Real powerCoeffReverse[nR,nS]
  ...
  Real K_eq_C[ nR] = K_eq_MWG "equilibrium constant";
 // Real A[nR];
equation
  for m in 1:nR loop
    for i in 1:nS loop
    ...
    K_eq_C[m] = product(C_reverse[m,:])/product(C_forward[m,:]);
    end for;
